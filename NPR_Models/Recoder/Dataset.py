...
class SumDataset(data.Dataset):
    def __init__(self, config, dataName="train",process_data_f="",nl_path="",val_nl_path="",val_process_data_f="",trndatapkl_f="",valdatapkl_f="",nl_voc_path="/root/zwk/Recoder_models/nl_voc.pkl",rule_path="/root/zwk/Recoder_models/rule.pkl",code_voc_path="/root/zwk/Recoder_models/code_voc.pkl",char_voc_path="/root/zwk/Recoder_models/char_voc.pkl"):
        ...
        self.Nls = []
        self.num_step = 50
        self.ruledict = pickle.load(open(rule_path, "rb"))
        ...
        #print(self.Nl_Voc)
        if dataName == "train":
            if os.path.exists(trndatapkl_f):
                print("loading training data")
                self.data = pickle.load(open(trndatapkl_f, "rb"))
                return
            else:
    ...
    def Load_Voc(self,nl_voc_path,code_voc_path,char_voc_path):
        if os.path.exists(nl_voc_path):
            self.Nl_Voc = pickle.load(open(nl_voc_path, "rb"))
        if os.path.exists(code_voc_path):
            self.Code_Voc = pickle.load(open(code_voc_path, "rb"))
        if os.path.exists(char_voc_path):
            self.Char_Voc = pickle.load(open(char_voc_path, "rb"))
