...
def write_oriLines(lines,file):
    with open(file,'w',encoding='utf8')as f:
        for line in lines:
            f.write(line)
        f.close()
def testone_deval(ssize, testmodel, filepath, lineid, classname, classcontent_file, valdatapkl_f, nl_voc_f, rule_f,
            code_voc_path, char_voc_path, rulead_path):
    model = testmodel
    st = time.time()
    ...
    if True:
        ...
        data.append(
            {'classname': classname, 'treeroot': treeroot, 'troot': troot, 'oldcode': oldcode, 'filepath': filepath,
             'subroot': subroot, 'vardic': vardic, 'typedic': typedic, 'precode': precode, 'aftercode': aftercode,
             'tree': troot.printTreeWithVar(troot, vardic), 'prob': troot.getTreeProb(troot), 'mode': 0, 'line': lineid,
             'isa': False})
    # print(time.time() - st)
    # print(time.time() - st)
    # print(2)
    if os.path.exists(classcontent_file):
        ...
        ans = solveone3(data, model, ssize, classcontent_file, valdatapkl_f, nl_voc_f, rule_f, code_voc_path,
                        char_voc_path, rulead_path)
    else:
...
def generate_fixes_deval_onehunk(model_path,bugs_info_path,search_size,output_dir,valdatapkl_f,nl_voc_f,rule_f,code_voc_path,char_voc_path,rulead_path,nl_voc_size,code_voc_size,voc_size,rule_num,cnum):
    model=load_model_for_test(model_path,nl_voc_size,code_voc_size,voc_size,rule_num,cnum)

    failed_ids=[]
    bugs_infos = readJson(bugs_info_path)
    #from clearml import Task
    #task = Task.init(project_name="NPR4j_translate", task_name="Recoder_bears")
    import BugCharacter as bc
    import data_process as dp
    def write_buggy_method(buggy_file_path,m_start_line,m_end_line,output_path):
        class_lines = read_oriLines(buggy_file_path)
        buggy_method_lines = class_lines[m_start_line:m_end_line+1]
        write_oriLines(buggy_method_lines,output_path)

    for idx,bug_id in enumerate(list(bugs_infos.keys())):
        modified_files = bugs_infos[bug_id]
        if len(modified_files) > 1:
            ...
            if not bc.isHunkContinual(buggy_line_ids):
                # TODO add logger for passing multi-hunk bugs within a file
                continue
                failed_ids.append(bug_id)
            else:
                start_line, end_line = buggy_line_ids[0], buggy_line_ids[-1]
                method_start_line, method_end_line = bc.find_buggymethod_java(file_path, start_line, end_line)
                if method_start_line == None:
                    continue
                    failed_ids.append(bug_id)
                file_name  = file_path.split('/')[-1].replace(".java.buggy","")
                class_content_path = os.path.join(output_dir,bug_id+".json")
                dp.generate_classcontent(file_path,class_content_path,file_name)#
                buggy_method_f = os.path.join(output_dir,bug_id+".buggy_method.txt")
                write_buggy_method(file_path,method_start_line,method_end_line,buggy_method_f)
                time.sleep(1)
                buggy_line_id = start_line-method_start_line
                try:
                    print("start fixing", bug_id,file_name,buggy_line_id)
                    fixs = testone(search_size, model, buggy_method_f, buggy_line_id, file_name,
                                   class_content_path, valdatapkl_f, nl_voc_f, rule_f, code_voc_path, char_voc_path,
                                   rulead_path)
                    fix_dict = {}
                    # get patch_code
                    for idx, fix in enumerate(fixs):
                        fix_dict[idx] = fix["code"]
                    patch_f = codecs.open(output_dir + '/' + bug_id + '.fix', 'w', encoding='utf8')
    ...
    write_oriLines(failed_ids,output_dir+'/failed_ids.txt')
