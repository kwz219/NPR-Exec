...
from utils import get_filenames, get_elapse_time, load_and_cache_gen_data, load_and_cache_gen_data_debugeval
    ...
    if args.do_test:
        ...
        for criteria in ['best-bleu']:
            ...
            if hasattr(model, 'module'):
                model.module.load_state_dict(torch.load(args.load_model_path))
            else:
                model.load_state_dict(torch.load(args.load_model_path))
            eval_examples, eval_data,id_map = load_and_cache_gen_data_debugeval(args, args.test_filename, pool, tokenizer, 'test',
                                                               only_src=True, is_sample=False)
            writeJson(id_map, os.path.join(args.output_dir,"id_map.json"))
