...
def main():
    ...
    if args.do_train:
        ...
        train_examples = prepare_examples_dir(args.train_dir)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval and (int(epoch) % 2 == 0 or int(epoch) > int(args.num_train_epochs) - 5 or int(epoch) < 5 ):
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples = prepare_examples_dir(args.dev_dir,mode="dev")
    ...
    if args.do_test:
        ...
        for idx, file in enumerate(files):
            ...
            eval_examples,id_bug_map =dp.readAPRexamples_single_hunk(readJson(args.test_filename))
            eval_features = convert_examples_to_features(eval_examples, tokenizer, args, stage='test')
            all_source_ids = torch.tensor([f.source_ids for f in eval_features], dtype=torch.long)
            # all_source_mask = torch.tensor([f.source_mask for f in eval_features], dtype=torch.long)
            eval_data = TensorDataset(all_source_ids)
            ...
            model.eval()
            p = []
            pred_f = codecs.open(os.path.join(args.output_dir, "test.output"), 'w', encoding='utf8')
            ...
            #dev_bleu = round(_bleu(os.path.join(args.output_dir, "test_{}.gold".format(str(args.test_filename))).format(file),
                                   #os.path.join(args.output_dir, "test_{}.output".format(str(args.test_filename))).format(file)), 2)
            #logger.info("  %s = %s " % ("bleu-4", str(dev_bleu)))
            #logger.info("  %s = %s " % ("xMatch", str(round(np.mean(accs) * 100, 4))))
            #logger.info("  " + "Test End" +"*" * 20)
            writeJson(id_bug_map,os.path.join(args.output_dir,"id_bug_match.json"))
