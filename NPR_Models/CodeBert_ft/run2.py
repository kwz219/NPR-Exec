...
from NPR_Models.CodeBert_ft.data_process import readAPRexamples_single_hunk
from NPR_Models.CodeBert_ft.model import Seq2Seq
...
def main():
    ...
    if args.do_train:
        ...
        train_examples = read_APRexamples(args.train_filename)
    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):   
            ...
            eval_examples = read_APRexamples(file)
            eval_features = convert_examples_to_features(eval_examples, tokenizer, args,stage='test')
            all_source_ids = torch.tensor([f.source_ids for f in eval_features], dtype=torch.long)
            all_source_mask = torch.tensor([f.source_mask for f in eval_features], dtype=torch.long)    
            eval_data = TensorDataset(all_source_ids,all_source_mask)   
            ...
            with open(os.path.join(args.output_dir,"test_{}.output".format(str(idx))),'w') as f, open(os.path.join(args.output_dir,"test_{}.gold".format(str(idx))),'w') as f1:
                for ref,gold in zip(p,eval_examples):
                    predictions.append(str(gold.idx)+'<SEP>'+ref)
                    f.write(str(gold.idx)+'<SEP>'+ref+'\n')
                    f1.write(str(gold.idx)+'<SEP>'+gold.target+'\n')
...
def mainWithArgs(gradient_accumulation_steps,train_batch_size,train_filename,do_train,train_steps,learning_rate,do_eval,eval_steps,test_filename,do_test,
                  ...
                  config_name,model_name_or_path,model_type,output_dir,bugs_info:dict,pred_file="./pred.txt",load_model_path=None,do_lower_case=False,local_rank=-1,no_cuda=False,n_gpu=1,seed=42):
    ...
    if do_train:
        # Prepare training data loader
        train_examples = read_APRexamples(train_filename)
        train_features = convert_examples_to_features(train_examples, tokenizer, max_source_length,max_target_length, stage='train')
        all_source_ids = torch.tensor([f.source_ids for f in train_features], dtype=torch.long)
        all_source_mask = torch.tensor([f.source_mask for f in train_features], dtype=torch.long)
        all_target_ids = torch.tensor([f.target_ids for f in train_features], dtype=torch.long)
        all_target_mask = torch.tensor([f.target_mask for f in train_features], dtype=torch.long)
        ...
        for step in bar:
            ...
            if do_eval and ((global_step + 1) % eval_steps == 0) and eval_flag:
                # Eval model with dev dataset
                tr_loss = 0
                nb_tr_examples, nb_tr_steps = 0, 0
                eval_flag = False
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples = read_APRexamples(dev_file_name)
                    eval_features = convert_examples_to_features(eval_examples, tokenizer, max_source_length,max_target_length, stage='dev')
                    all_source_ids = torch.tensor([f.source_ids for f in eval_features], dtype=torch.long)
                    all_source_mask = torch.tensor([f.source_mask for f in eval_features], dtype=torch.long)
                    all_target_ids = torch.tensor([f.target_ids for f in eval_features], dtype=torch.long)
                    all_target_mask = torch.tensor([f.target_mask for f in eval_features], dtype=torch.long)
    ...
    if do_test:
        ...
        if dev_file_name is not None:
            files.append(dev_file_name)
        if test_filename is not None:
            files.append(test_filename)
        for idx, file in enumerate(files):
            logger.info("Test file: {}".format(file))
            eval_examples = readAPRexamples_single_hunk(bugs_info)
