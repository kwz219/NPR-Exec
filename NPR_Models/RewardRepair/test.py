...
def test(output_f, tokenizer, model, device, loader):
    return_sequences = 300
    model.eval()
    ...
    with torch.no_grad():
        for _, data in enumerate(loader, 0):
            ...
            with open(output_f, 'a') as csvfile:
...
def main(input_f,output_f,model_dir):
    ...
    test_df = pd.read_csv(input_f,encoding='utf-8',delimiter='\t')
    print(test_df.head())
    test_df = test_df[['bugid','store_id','buggy','patch']]
    print(test_df.head())
    ...
    test(output_f, tokenizer, model, device, test_loader)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-input_f",help="")
    parser.add_argument("-output_f", help="")
    parser.add_argument("-model_dir", help="")
    opt = parser.parse_args()
    main(opt.input_f,opt.output_f,opt.model_dir)
